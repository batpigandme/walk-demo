...
#| label: deauth
#| echo: false
# since we're using data built into googlesheets4 we don't need to do auth
gs4_deauth()
```
...
The backslash syntax for anonymous functions (e.g. `\(x) x + 1`) was introduced in base R version 4.1.0 as a shorthand for `function(x) x + 1`.
